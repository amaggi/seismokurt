...
def Fast_Kurtogram(x, nlevel, Fs=1, opt1=None, opt2=None):
    ...
    if opt1 == 1:
        ...
        plt.imshow(np.sqrt(Kwav),aspect='auto',extent=(freq_w[0],freq_w[-1],Level_w[0],Level_w[-1]),interpolation='none')
    ...
    lev = l1
    while test == 1:
        #~ fi = float(raw_input('	Enter the optimal carrier frequency where to filter the signal: '))
        #~ fi = fi/Fs
        fi = fi
        if opt1 == 1:
